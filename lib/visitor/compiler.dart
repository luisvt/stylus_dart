...
Compiler.prototype.__defineGetter__('indent', (){
  if (this.compress) return '';
  return new List(this.indents).join(List(this.spaces + 1).join(' '));
});
...
visitGroup(group) {
  ...
  if (group.block.hasProperties) {
    var selectors = super.compileSelectors( stack)
...
visitExpression(expr) {
  ...
  var buf = []
    ...
    , nodes = expr.nodes.map((node){ return self.visit(node); });
