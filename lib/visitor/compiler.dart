...
class Compiler extends Visitor {
  ...
  Compiler(root, [options]) : super(root) {
    ...
    this.spaces = or(options['indent spaces'], 2);
    this.indents = 1;
    this.stack = [];
  }
  ...
  get indent {
    ...
    return new List(this.indents).join(new List(this.spaces + 1).join(' '));
  }
  ...
  visitBlock(block) {
    ...
    if (block.hasProperties && !block.lacksRenderedSelectors) {
      ...
      for (var i = 0, len = block.nodes.length; i < len; ++i) {
        ...
        // inline comments
          case 'comment':
            if (!this.compress && node.inline) {
              this.buf = this.buf.substring(0, -1);
  ...
  visitLiteral(lit) {
    var val = lit.val;
    if (lit.css) val = val.replace(new RegExp(r'^  /g'), '');
    return val;
  }
  ...
  visitUnit(unit) {
    ...
    if (this.compress) {
      ...
      if (float && n < 1 && n > -1) {
        return n.toString().replaceFirst('0.', '.') + type;
      }
    }

    return (float ? double.parse(n.toFixed(15)) : n).toString() + type;
  }
  ...
  visitGroup(group) {
    ...
    if (group.block.hasProperties) {
      var selectors = utils.compileSelectors(stack),
  ...
  visitCall(call) {
    ...
    var args = call.args.nodes.map((arg) {
  ...
  visitArguments(expr) => visitExpression(expr);
  ...
  debugInfo(node) {
    var path = node.filename == 'stdin' ? 'stdin' : fs.realpathSync(node.filename),
        line = or((node.nodes && node.nodes.length ? node.nodes[0].lineno : node.lineno), 1);
    ...
    if (this.firebug) {
      ...
      path = 'file\\\:\\\/\\\/' + path.replace(new RegExp(r'([.:/\\])/'), (m) {
        return '\\' + (m == '\\' ? '\/' : m);
      });
      line = '\\00003' + line;
