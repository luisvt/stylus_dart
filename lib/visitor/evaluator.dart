...
import 'package:stylus_dart/functions/index.dart' show bifs;
...
import 'package:stylus_dart/units.dart';
...
//    , Parser = require('../parser')
  ...
  setup() {
    ...
    this.imports.forEach((file) {
      ...
      expr.add(new nodes.$String(file));
      imports.add(new nodes.Import(expr));
    }, this);
  ...
  visitGroup(group) {
    group.nodes = group.nodes.map((selector) {
  ...
  visitQueryList(queries) {
    ...
    if (1 == queries.nodes.length) {
      ...
      if (val = this.lookup(query.type)) {
        ...
        var parser = new Parser(val, this.options);
  ...
  visitMember(node) {
    ...
    if ('object' != obj.nodeName) {
      throw new Exception(left.toString() + ' has no property .' + right);
    }
    if (node.val) {
      this.$return++;
  ...
  visitEach(each) {
    ...
    return or(vals[vals.length - 1], nodes.$null);
  }
  ...
  visitArguments(exp) => visitExpression(exp);
  ...
  visitBlock(block) {
    this.stack.add(new Frame(block));
  ...
  visitExtend(extend) {
    ...
    extend.selectors.forEach((selector) {
      block.node.$extends.add({
  ...
  visitImport(imported) {
    ...
    // url() passed
    if ('url' == path.name) {
      if (imported.once) throw new Exception('You cannot @require a url');

      return imported;
    }

    // Ensure string
    if (!path.string) throw new Exception('@' + nodeName + ' string expected');

    var name = path = path.string;

    // Absolute URL or hash
    if (new RegExp('(?:url\\s*\(\\s*)?[\'"]?(?:#|(?:https?:)?\\/\\/)/').hasMatch(path)) {
      if (imported.once) throw new Exception('You cannot @require a url');
      return imported;
    }
    ...
    if (!literal && !new RegExp(r'\.styl$/').hasMatch(path)) path += '.styl';
    ...
    if (!found) throw new Exception('failed to locate @' + nodeName + ' file ' + path);

    var block = new nodes.Block();

    for (var i = 0, len = found.length; i < len; ++i) {
      block.add(importFile.call(this, imported, found[i], literal));
    }
  ...
  invokeFunction(fn, args, content) {
    ...
    scope.add(new nodes.Ident('mixin', truthy(this.$return)
        ? nodes.$false
    ...
    for (var i = this.calling.length - 1; i--;) {
      expr.add(new nodes.Literal(this.calling[i]));
    };
    ...
    fn.params.nodes.forEach((node) {
      ...
      if (node.rest) {
        ...
        if (node.val.isNull) {
          throw new Exception('argument "' + node + '" required for ' + fn);
        }
      }
  ...
  invokeBuiltin(fn, args) {
    ...
    if (fn.raw) {
      ...
      args = utils.params(fn).reduce((ret, param) {
        var arg = or(args.map[param], shift(args.nodes));
        if (arg) {
          arg = utils.unwrap(arg);
          var len = arg.nodes.length;
          if (len > 1) {
            for (var i = 0; i < len; ++i) {
              ret.add(utils.unwrap(arg.nodes[i].first));
            }
          } else {
            ret.add(arg.first);
          }
  ...
  invoke(body, [stack, filename]) {
    ...
    if (filename) this.paths.add(dirname(filename));

    // Return
    if (truthy(this.$return)) {
      ret = this.eval(body.nodes);
      if (stack) pop(this.stack);
      // Mixin
    } else {
  ...
  mixin(nodes, block) {
    ...
    ,
        tail = slice(block.nodes, block.index + 1, len);
  ...
  get selectorStack {
    ...
    for (var i = 0, len = this.stack.length; i < len; ++i) {
      block = this.stack[i].block;
      if (block.node && 'group' == block.node.nodeName) {
        block.node.nodes.forEach((selector) {
          if (!selector.val) selector.val = this.interpolate(selector);
        }, this);
        stack.add(block.node.nodes);
      }
    }
    return stack;
  }
  ...
  lookup(name) {
    var val;
    if (this.ignoreColors && colors.containsKey(name)) return null;
      ...
      switch (node.nodeName) {
        ...
        case 'string':
          if (self.prefix && !node.prefixed && !node.val.nodeName) {
            node.val = node.val.replace(new RegExp(r'\./'), '.' + self.prefix);
            node.prefixed = true;
          }
          return node.val;
        case 'unit':
  ...
  lookupFunction(name) {
    var fn = or(this.functions[name], bifs[name]);
  ...
  isDefined(node) {
    if ('ident' == node.nodeName) {
      ...
      throw new Exception('invalid "is defined" check on non-variable ' + node);
    }
  }
  ...
  propertyExpression(prop, name) {
    ...
    // name
    expr.add(new nodes.$String(prop.name));
