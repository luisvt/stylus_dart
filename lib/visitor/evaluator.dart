...
visitImport(imported) {
  ...
  if (new RegExp(r'(?:url\s*\(\s*)?['"]?(?:#|(?:https?:)?\/\/)/').test(path)) {
    if (imported.once) throw new Error('You cannot @require a url');
    return imported;
  }
  ...
  if (!literal && !new RegExp(r'\.styl$/').test(path)) path += '.styl';
...
invokeFunction(fn, args, content) {
  ...
  fn.params.nodes.forEach((node){
    ...
    if (node.rest) {
      ...
      for (; i < len; ++i) node.val.add(args.nodes[i]);
...
Evaluator.prototype.__defineGetter__('closestBlock', (){
  var i = this.stack.length
    , block;
  while (i--) {
    block = this.stack[i].block;
...
Evaluator.prototype.__defineGetter__('closestGroup', (){
  var i = this.stack.length
    , block;
  while (i--) {
    block = this.stack[i].block;
...
Evaluator.prototype.__defineGetter__('selectorStack', (){
  var block
    , stack = [];
...
lookup(name) {
  ...
  var val;
  if (this.ignoreColors &&  colors.containsKey(name)) return;
...
Evaluator.prototype.__defineGetter__('currentBlock', (){
...
Evaluator.prototype.__defineGetter__('vendors', (){
  return this.lookup('vendors').nodes.map((node){
    return node.string;
  });
});
...
Evaluator.prototype.__defineGetter__('currentScope', (){
...
Evaluator.prototype.__defineGetter__('currentFrame', (){
