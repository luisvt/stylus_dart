...
class Parser extends JsonObject {
  ...
  this.prefix = or(options.prefix, '');
  this.root = or(options.root, new nodes.Root);
  this.state = ['root'];
  this.stash = [];
  this.parens = 0;
  this.css = 0;
...
static getCache(options) {
  ...
  looksLikeSelector([fromProperty]) {
    ...
    while (or(this.isSelectorToken(i), ',' == this.lookahead(i).type)) {
  ...
  statement() {
    ...
    switch (state) {
      ...
      case 'function arguments':
        while (op =
             or(this.accept('if'), this.accept('unless'))
          || this.accept('for')) {
          switch (op.type) {
            case 'if':
            case 'unless':
  ...
  $if() {
    ...
    while (this.accept('else')) {
      if (this.accept('if')) {
        ...
        node.elses.add(new nodes.If(cond, block));
      } else {
        node.elses.add(this.block(node, false));
        break;
      }
      this.skip(['newline', 'comment']);
    }
  ...
  query() {
    ...
    if ('ident' == this.peek().type
      ...
      this.cond = false;
      query.add(new nodes.Feature(expr.nodes));
      return query;
    }

    if (pred = or(this.accept('ident'), this.accept('not'))) {
  ...
  interpolate() {
    ...
    star = this.accept('*');
    if (star) segs.add(new nodes.Literal('*'));

    while (true) {
      if (this.accept('{')) {
        this.state.add('interpolation');
        segs.add(this.expression());
        this.expect('}');
        pop(this.state);
      } else if (node = this.accept('-')){
        segs.add(new nodes.Literal('-'));
  ...
  selector() {
    ...
    do {
      ...
      // Push the selector
      if (isRoot && scope) unshift(arr, new nodes.Literal(scope + ' '));
      if (arr.length) {
    ...
    } while (or(this.accept(','), this.accept('newline')));

    if ('selector-parts' == this.currentState()) return group.nodes;

    this.state.add('selector');
  ...
  selectorParts(){
    ...
    while (tok = this.selectorToken()) {
      ...
      switch (tok.type) {
        ...
        case 'color':
        case 'unit':
          arr.add(new nodes.Literal(tok.val.raw));
          break;
        case 'space':
          arr.add(new nodes.Literal(' '));
          break;
        case 'function':
          arr.add(new nodes.Literal(tok.val.name + '('));
          break;
        case 'ident':
          arr.add(new nodes.Literal(tok.val.name || tok.val.string));
          break;
        default:
          arr.add(new nodes.Literal(tok.val));
          if (tok.space) arr.add(new nodes.Literal(' '));
      }
    }
  ...
  assignment() {
    ...
    if (op =
         or(this.accept('='), this.accept('?='))
      || this.accept('+=')
      || this.accept('-=')
      || this.accept('*=')
      || this.accept('/=')
      || this.accept('%=')) {
      this.state.add('assignment');
      var expr = this.list();
      // @block support
  ...
  functionDefinition() {
    ...
    // params
    this.state.add('function params');
    this.skipWhitespace();
  ...
  object(){
    ...
    while (!this.accept('}')) {
      if (or(this.accept('comment'), this.accept('newline'))) continue;

      if (!comma) this.accept(',');
      id = or(this.accept('ident'), this.accept('string'));
  ...
  primary() {
    ...
    if (this.accept('(')) {
      ...
      var expr = this.expression()
        , paren = this.expect(')');
      --this.parens;
      if (this.accept('%')) expr.add(new nodes.Ident('%'));
      tok = this.peek();
      // (1 + 2)px, (1 + 2)em, etc.
      if (!paren.space
        && 'ident' == tok.type
        && ~units.indexOf(tok.val.string)) {
        expr.add(new nodes.Ident(tok.val.string));
        this.next();
      }
      return expr;
    }
